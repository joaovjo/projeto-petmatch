...
// Enums
export const especieEnum = pgEnum("especie_enum", ["Cachorro", "Gato", "Outro"]);
...
export const pet = pgTable(
  "pet",
  {
    ...
    ongId: uuid("ong_id").notNull().references(() => ong.id, {
      onDelete: "cascade",
    }),
  ...
  (table) => [
    ...
    check("pet_owner_check", sql`${table.ongId} IS NOT NULL OR ${table.tutorId} IS NOT NULL`),
